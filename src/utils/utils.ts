...
export function message(type, custom = '') {
  ...
  【智能引导】：
  1. 汇总记录时，智能摘要，分析和汇总开票日期和项目名称相同的记录
  ...
  const taskPrompts = {
    'fapiao': `
      从发票内容提取重要字段，以合法JSON格式输出，关键信息包括金额、税号、日期、销售方、购买方、发票类型，不要项目明细，把所有项目归类为一个项目。
      最终输出格式(严格遵守)：<JSON object[]>

      注意：必须包括一个项目名称。仅输出要求内容，无额外文本。
    `,

    'csv-summary-header': `
     【任务开始】：
      任务分两步，**必须按顺序完成**：
      1. **强制要求 - 智能汇总**：将发票数据按"开票日期"+"项目名称"两个维度进行**合并汇总**， 每条汇总要包括发票张数。
      **必须保存**为JSON格式。
      2. **强制要求**：基于第1步汇总结果和“Excel模版中提取出的列定义”，**必须生成**表格, 单元格无数据时，该单元格则填写'--'。
      
      **最终输出格式(严格遵守，这是核心要求)：**
      [{ "summary": summaryData }, { "csv": csv格式Data }]

      注意(严格遵守)：
      1. 仅输出要求内容，无任何额外文本；
      2. 汇总后的数据应该是原始数据的精简汇总版本；
      3. **核心规则**：日期相同且项目名称相同的记录必须合并汇总成一条；
    ...
    'csv-summary': `
     【任务开始】：
      ...
      1. **强制要求 - 智能汇总**：将发票数据按"开票日期"+"项目名称"两个维度进行**合并汇总**， 每条汇总要包括发票张数。
      **必须保存**为JSON格式。
      2. **强制要求**：基于第1步汇总结果和报销规范智能生成表头（常见字段包括日期、金额、商户、类别等），**必须生成**表格, 单元格无数据时，该单元格则填写'--'。
    ...
    'csv-header': `
     【任务开始】：
      仅基于当前提供的发票图像内容，根据发票数据和Excel模版中提取出的列定义，生成表格, 单元格无数据时，该单元格则填写'--'。
      并保存[{"csv": csv格式Data}]。
    ...
    'csv': `
     【任务开始】：
      仅基于当前提供的发票图像内容，根据发票数据和报销规范智能生成表头（常见字段包括日期、金额、商户、类别等），生成表格, 单元格无数据时，该单元格则填写'--'。
      并保存[{"csv": csv格式Data}]。
