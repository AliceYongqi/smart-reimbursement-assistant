...
export function message(type, custom = '') {
  ...
  2. 商品类别必须严格按照提供的数据中的实际类别填写
  3. 关键财务数据必须完全匹配提供的数据，不得估算
  4. 禁止添加任何解释、注释或额外文本
  5. 只输出要求的内容，前后不要加任何字符
  6. 进行摘要或汇总时，只能使用提供的发票JSON数据中的信息
  7. 合并记录时，只能合并日期和项目名称完全相同的记录
  8. 生成的表头必须完全基于提供的数据，不得凭空创建不存在的表头字段

  【智能引导】：
  1. 当没有提供Excel模板时，可以基于发票实际内容智能生成合理的表头
  2. 表头字段应覆盖发票上的所有关键信息，并遵循财务报表的通用规范
  3. 可以根据发票类型调整表头字段的优先级和组合
  4. 如果发票上有特殊字段，应包含在表头中
  `;
  ...
  const taskPrompts = {
    ...
    'csv-summary-header': `
      任务分两步：
      1. **强制要求**：将日期和项目名称完全相同的多条记录（例如2024年09月18日的运输服务*客运服务费）合并成一条汇总记录。汇总时，金额等数值字段累加计算，其他共同字段保持不变。保存为JSON格式 [{ "summary": json格式data }]。
      2. 根据第1步汇总结果和Excel模版中提取出的列定义，生成表格并保存[{ "csv": CSV格式data }]。
      
      最终输出格式(严格遵守)：
      把第1步和第2步的结果合并成一个object[]并输出。

      注意(严格遵守)：
      1. 仅输出要求内容，无任何额外文本
      2. **绝对不允许**重复输出原始记录
      3. 汇总后的数据应该是原始数据的精简汇总版本
      4. 对于日期和项目名称相同的记录，无论其他字段是否有细微差异，都必须合并汇总
    `,
    'csv-summary': `
      任务分两步：
      1. **强制要求**：将日期和项目名称完全相同的多条记录（例如2024年09月18日的运输服务*客运服务费）合并成一条汇总记录。汇总时，金额等数值字段累加计算，其他共同字段保持不变。保存为JSON格式 [{ "summary": json格式data }]。
      2. 根据第1步汇总结果和报销规范智能生成表头（常见字段包括日期、金额、商户、分类、税号、项目名称等），生成表格并保存[{"csv": CSV格式data}]
    ...
    `,
    'csv': `
      仅基于当前提供的发票图像内容，根据发票数据和报销规范智能生成表头（常见字段包括日期、金额、商户、分类、税号、项目名称等），生成表格并保存[{"csv": CSV格式data}]。
