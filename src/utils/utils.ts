...
export function message(type, custom = '') {
  ...
  const taskPrompts = {
    ...
    'csv-summary-header': `
      任务分两步：
      1. **仅基于提供的发票JSON数据**，对发票数据进行分析、统计、汇总，然后**只合并日期和项目名称完全相同**的记录成一条，保存JSON格式 [{ "summary": json格式data }], 确保输出的JSON格式正确。
	    2. 根据第1步保存的结果和Excel模版中提取出的列定义（包括但不限于第一行），生成表格（**严格按照提供的数据填写，不要添加任何不存在的信息**）并保存[{ "csv": CSV格式data }]。
      ...
      3. 合并记录时必须严格匹配日期和项目名称，不得随意合并不同的记录
    `,
    'csv-summary': `
      任务分两步：
      1. **仅基于提供的发票JSON数据**，对发票数据进行分析、统计、汇总，然后**只合并日期和项目名称完全相同**的记录成一条，保存JSON格式 [{ "summary": json格式data }], 确保输出的JSON格式正确。
	    2. 根据第1步保存的结果和报销规范智能生成表头（常见字段包括但不限于：日期、金额、商户、分类、税号、项目名称等），生成表格（**严格按照提供的数据填写，不要添加任何不存在的信息**）并保存[{"csv": CSV格式data}]。
